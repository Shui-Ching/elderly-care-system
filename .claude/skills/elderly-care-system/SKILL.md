...
### 3. 社工後台 `social-worker`
- **狀態：** ✅ 已完成（SPA 架構，6 個面板全數完成）
- **平台：** Web（桌機優先）
- **技術棧：** HTML / SCSS / JS
- **已完成面板（`social-worker.html`）：**
  - `dashboard` 儀表板總覽 — 統計卡 ×4、長者狀態列表、警報面板、今日行程
  - `elders` 我的長者 — 搜尋篩選、風險分數列表
  - `alerts` 警報中心 — 摘要橫幅、篩選、警報卡（含操作鈕）
  - `visits` 探訪紀錄 — 月曆 + 當日日程雙欄
  - `reports` 月報產出 — 統計卡、長條圖（CSS）、圓環圖（SVG）、趨勢表格
  - `settings` 系統設定 — 個人資訊、通知偏好 Toggle、系統資訊、登出

### 4. 管理員後台 `admin-panel`
- **狀態：** ✅ 已完成（SPA 架構，5 個面板全數完成）
- **平台：** Web（桌機）
- **技術棧：** HTML / SCSS / JS
- **已完成面板（`admin.html`）：**
  - `dashboard` 系統總覽 — 統計卡 ×4（系統狀態/裝置/用戶/警報）、子系統健康清單、近期事件 Feed
  - `rules` 警報規則設定 — 6 條規則卡片（閾值輸入、等級徽章、觸發動作、Toggle 啟停）
  - `accounts` 帳號管理 — 搜尋篩選、角色徽章（管理員/社工/志工/家屬）、停用/啟用
  - `devices` 裝置管理 — 摘要磁磚、裝置列表（電量條視覺化、狀態徽章、異常行亮色）
  - `logs` 系統日誌 — 篩選列（警報/登入/裝置/系統/設定）、帶嚴重度色條的事件 Feed
